 ...
 * @param {String[]|String} [options.wordbreak] The wordbreak strings to truncate to. Either an array of strings or a single string
 *
 ...
 * @returns {String|Void} Returns the truncated string, or undefined for undefined input
 *
 * Based on the lodash Truncate Function
...
const truncate = (value, options) => {
  ...
  if (localOptions.wordbreak && _.isArray(localOptions.wordbreak)) {
    localOptions.separator = new RegExp(`[${localOptions.wordbreak.join('')}]`);
  }

  if (localOptions.wordbreak && _.isString(localOptions.wordbreak)) {
